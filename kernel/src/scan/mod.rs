...
mod tests {
    ...
    fn test_scan_files() {
        ...
        let scan = ScanBuilder::new(snapshot).build(table_client.as_ref()).unwrap();
        let files: Vec<Add> = scan.files(table_client.as_ref()).unwrap().try_collect().unwrap();
    ...
    fn test_scan_data() {
        ...
        let snapshot = table.snapshot(table_client.as_ref(), None).unwrap();
        let scan = ScanBuilder::new(snapshot).build(table_client.as_ref()).unwrap();
        let files = scan.execute(table_client.as_ref()).unwrap();
